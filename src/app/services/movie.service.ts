...
export class MovieService {
  ...
  private loadMovies(): void {
    this.http.get<{ results: any[] }>(this.moviesEndpoint).pipe(
      ...
      this.loadAllMovieDetails(populatedMovies);
    });
  }

  private loadAllMovieDetails(movies: MovieProjection[]): void {
    const detailRequests = movies.map(movie =>
      this.getMovieDetailsFromTmdb(movie.id).pipe(
        map(details => ({
          ...movie,
    ...
    forkJoin(detailRequests).subscribe(updatedMovies => {
      const moviesNeedingDescription = updatedMovies.filter(m => !m.description);
      if (moviesNeedingDescription.length > 0) {
        const descRequests = moviesNeedingDescription.map(movie =>
          this.http.get<any>(`${this.BASE_URL}/movie/${movie.id}?api_key=${this.API_KEY}&language=en-US`).pipe(
            map(response => ({ id: movie.id, description: response.overview || '' })),
            catchError(() => of({ id: movie.id, description: '' }))
          )
        );
        forkJoin(descRequests).subscribe(descriptions => {
          const descMap = new Map(descriptions.map(d => [d.id, d.description]));
          const finalMovies = updatedMovies.map(movie => ({
            ...movie,
            description: movie.description || descMap.get(movie.id) || ''
          }));
          this.moviesSubject.next(finalMovies);
          console.log('Svi detalji filmova učitani (sa en-US fallback za opise).');
        });
      } else {
        this.moviesSubject.next(updatedMovies);
        console.log('Svi detalji filmova učitani.');
      }
    });
  }
      ...
      map(movies => {
        return movies.filter(movie => {
          ...
          if (criteria.releaseDate) {
            if (movie.releaseDate !== criteria.releaseDate) {
              match = false;
            }
          }
          if (criteria.projectionDate || criteria.projectionTime) {
            ...
            for (const pd of movie.projectionDates) {
              let dateMatch = !criteria.projectionDate || pd.date === criteria.projectionDate;
