...
export class MovieDetailComponent implements OnInit {
  ...
  onDateChange(movie: MovieProjection): void {
    this.selectedTime = '';
    const projection = movie.projectionDates.find(pd => pd.date === this.selectedDate);
    this.availableTimes = projection ? projection.times : [];
  }
  ...
  addMovieToCart(movie: MovieProjection): void {
    if (!this.selectedDate || !this.selectedTime) {
      Swal.fire({
        icon: 'warning',
        title: 'Izaberite projekciju!',
        text: 'Morate izabrati datum i vreme projekcije pre dodavanja u korpu.',
        timer: 3000,
