...
export class CartComponent implements OnInit {
  ...
  purchaseItems(): void {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        this.cartService.purchaseItems().subscribe({
          next: success => {
            if (success) {
              Swal.fire(
                'Uspešna kupovina!',
                'Vaša porudžbina je uspešno obrađena! Pogledajte vaše karte u "Moje Karte".',
                'success'
              );
              ...
              Swal.fire({
                icon: 'error',
                title: 'Greška!',
                text: 'Došlo je do greške prilikom kupovine.',
                timer: 3000,
                showConfirmButton: false
              });
            }
